...
get "/youtube/:channel_id/:username.ics" do |channel_id, username|
  ...
  data, _, etag = App::Cache.cache("youtube.ics", channel_id, 30, 30, env["HTTP_IF_NONE_MATCH"]) do
...
get %r{/instagram/(?<user_id>\d+)/(?<username>.+)} do |user_id, username|
  ...
  data, @updated_at, etag = App::Cache.cache("instagram.posts", user_id, 30*60, 30*60, env["HTTP_IF_NONE_MATCH"]) do
